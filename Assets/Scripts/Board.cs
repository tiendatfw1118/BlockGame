...
public class Board : MonoBehaviour
{
    ...
    Tile[,] m_allTiles;
    GamePiece[,] m_allGamePieces; // mảng lưu lại các gamepieces
    Tile m_clickedTile;
    Tile m_targetedTile;
    ...
    {
        int maxInterations = 100; // tránh infinite loop khi chỉ có 1 piece prefabs
        int interation = 0;
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
    ...
    {
        if (m_clickedTile != null && m_targetedTile != null)
        {
            SwitchTiles(m_clickedTile, m_targetedTile);
        }
    ...
    void HighlighPiece(List<GamePiece> gamePieces)
    {
        foreach (GamePiece piece in gamePieces)
        {
            if (piece != null)
    ...
    {
        List<GamePiece> movingPieces = new List<GamePiece>();
        List<int> columnToCollapse = GetColumns(gamePieces);
    ...
    IEnumerator ClearAndCollapseRoutine(List<GamePiece> gamePieces)
    {
        ...
        while (!isFinished)
        {
            ...
            matches = FindMatchesAt(movingPieces); // tìm Piece trùng trong hàng "đã" rơi xuống
    ...
    void TestDeadlock()
    {
        if (m_allGamePieces != null && m_boardDeadlock != null)
        {
            if (m_boardDeadlock.IsDeadlocked(m_allGamePieces, 3))
            {
                gameManager.OutOfMove();
            }
        }
    }
}
